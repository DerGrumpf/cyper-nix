...
A unified Nix flake managing NixOS desktops and a home server cluster — all sharing a common Home Manager configuration.
...
# Or use the shell alias (auto-detects hostname)
nix-switch
```
...
```bash
nix-check
# expands to: nix flake check --no-build
```
